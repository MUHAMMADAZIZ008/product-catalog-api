import { z } from "zod";

const productSchema = z.object({
    id: z.string().uuid().optional(), 
    name: z.string().min(1, '"name" majburiy maydon').nonempty('"name" bo\'sh bo\'lishi mumkin emas'),
    short_description: z.string().optional(),
    full_description: z.string().optional(),
    category_id: z.string().uuid().optional(), 
    seller_id: z.string().uuid().optional(), 
    price: z.decimal().min(0, '"price" musbat raqam bo\'lishi kerak').nonnegative('"price" 0 dan kichik bo\'lmasligi kerak'),
    discount_price: z.decimal().optional().refine(val => val === undefined || val >= 0, {
        message: '"discount_price" 0 dan kichik bo\'lmasligi kerak',
    }),
    quantity: z.number().int().nonnegative().default(0), 
    status: z.enum(['active', 'sold', 'on_sale']).default('active'),
    image_urls: z.array(z.string().url()).optional(), 
    created_at: z.string().optional(), 
    updated_at: z.string().optional(), 
});


const validateProduct = (product) => {
    return productSchema.safeParse(product);
};

