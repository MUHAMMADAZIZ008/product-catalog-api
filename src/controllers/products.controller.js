...
export const  getallProductController = async (req,res,next) => {
    try {
        ...
        if(products.length === 0){
            return res.status(404).send({status:'Not Found', message : 'No products found'} )

        return res.status(200).send({Status: 'Success', page, limit,products})
        }
    } catch (error) {
        next(error)
...
export const  getoneProductController = async (req,res,next) => {
    try {
        ...
        if(!product){
            return res.status(404).send({status:'Not Found', message : 'No product found'} )
